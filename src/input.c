...
int readParams(struct params *pars, const char *fname) {
     pars->SmoothingRadius = ini_getd("Finder", "SmoothingRadius", 1.0, fname);
     pars->ExclusionRadius = ini_getd("Finder", "ExclusionRadius", 2.0, fname);
     pars->MaximumPointDensity = ini_getd("Finder", "MaximumPointDensity", -0.8, fname);
     pars->DensityThreshold = ini_getd("Finder", "DensityThreshold", -0.8, fname);

     pars->MinRadius = ini_getd("AverageProfile", "MinRadius", -1, fname);
